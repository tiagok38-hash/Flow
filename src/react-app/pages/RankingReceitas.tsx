...
import { useReceitasPorCategoria, formatarMoeda } from '@/react-app/hooks/useApi';
...
export default function RankingReceitas() {
    const [periodo, setPeriodo] = useState('mes-atual');
    const [tipoGrafico, setTipoGrafico] = useState<'pizza' | 'barras'>('pizza');
    const { data: receitasPorCategoria, loading } = useReceitasPorCategoria(periodo);

    // Pegar apenas as top 3 categorias
    const top3Categorias = receitasPorCategoria?.slice(0, 3) || [];
            ...
            <div className="bg-gradient-to-br from-teal-50/60 to-emerald-50/60 shadow-lg shadow-teal-500/25">
                <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-xl font-light text-gray-900">Ranking de categorias (receitas)</h1>
                        </div>
                    </div>
                </div>
            </div>
                ...
                {loading ? (
                    ...
                    <div className="space-y-8">
                        ...
                        <Card className="bg-white/80 backdrop-blur-sm shadow-xl shadow-gray-400/30 animate-slide-up max-w-4xl mx-auto">
                            <h3 className="text-lg font-light text-gray-900 mb-6 text-center">Top 3 Receitas</h3>
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="text-center">
                                                ...
                                                <div className="text-center">
                                                    <p className="text-sm sm:text-lg font-bold text-teal-600 mb-1">
                                                        {formatarMoeda(categoria.total)}
                                                    </p>

                                                    {receitasPorCategoria && receitasPorCategoria.length > 1 && (
                                                        <p className="text-xs text-gray-500 font-light">
                                                            {((categoria.total / receitasPorCategoria.reduce((acc, cat) => acc + cat.total, 0)) * 100).toFixed(1)}% do total
                                                        </p>
                                                    )}
                                                </div>
                                            </div>
                                    ...
                                    <div className="flex flex-col md:flex-row items-center justify-center gap-8 py-4">
                                        <div className="w-full md:w-1/2 h-64 sm:h-72">
                                            <ResponsiveContainer width="100%" height="100%">
                                                <RechartsPieChart>
                                                    <Pie
                                                        data={receitasPorCategoria.map((item, index) => ({
                                                    ...
                                                    <Tooltip
                                                        content={({ active, payload }) => {
                                                            if (active && payload && payload.length) {
                                                                ...
                                                                const total = receitasPorCategoria.reduce((acc, item) => acc + item.total, 0);
                                                                const percentual = ((data.value / total) * 100).toFixed(1);
                                                                const categoria = receitasPorCategoria.find(cat => cat.categoria_nome === data.payload.name);
                                                                    ...
                                                                    <div className="bg-white/95 backdrop-blur-md p-4 rounded-2xl shadow-2xl border border-gray-100 min-w-[200px] animate-fade-in">
                                                                        ...
                                                                        <div className="space-y-2">
                                                                            <div className="flex justify-between items-center">
                                                                                <span className="text-xs text-gray-500 font-light tracking-wide">Valor total</span>
                                                                                <span className="text-teal-600 font-bold text-sm">{formatarMoeda(data.value)}</span>
                                                                            </div>
                                                    ...
                                                    if (active && payload && payload.length) {
                                                        ...
                                                        return (
                                                            <div className="bg-white/95 backdrop-blur-md p-3 rounded-2xl shadow-xl border border-gray-100 animate-fade-in min-w-[140px]">
                                                                ...
                                                                <p className="text-sm font-bold text-teal-600">{formatarMoeda(data.value)}</p>
                                                            </div>
                                                        );
                                                    }
                                                    return null;
                        ...
                        {receitasPorCategoria && receitasPorCategoria.length > 3 && (
                            <Card className="bg-white/70 backdrop-blur-sm shadow-xl shadow-gray-400/30 animate-slide-up-delay-2">
                                <h3 className="text-lg font-light text-gray-900 mb-6">Outros Recebimentos</h3>

                                <div className="space-y-4">
                                    {receitasPorCategoria.slice(3).map((categoria, index) => {
                        ...
                        <Card className="bg-white/70 backdrop-blur-sm shadow-xl shadow-gray-400/30 animate-slide-up-delay-3">
                            <div className="flex items-center justify-between">
                                <div>
                                    <h3 className="text-lg font-light text-gray-900 mb-2">Total Recebido</h3>
                                ...
                                <div className="text-right">
                                    <p className="text-3xl font-bold text-teal-600">
                                        {formatarMoeda(receitasPorCategoria?.reduce((acc, cat) => acc + cat.total, 0) || 0)}
                                    </p>
                                    <p className="text-sm text-gray-500 font-light">
                                        {receitasPorCategoria?.length || 0} categorias
                                    </p>
                                </div>
                            </div>
                        </Card>
